...
const AdminDashboard = () => {
  ...
  const handleOverrideResult = async (e) => {
    ...
    try {
      await api.put(
        `/admin/matches/${selectedMatch._id}/override`,
        overrideData,
      ); // [F7] fixed to PUT
    ...
    switch (activeTab) {
      ...
      case "matches":
        return matches.filter(
          (m) =>
            m.team_a_id?.name.toLowerCase().includes(query) ||
            m.team_b_id?.name.toLowerCase().includes(query),
        );
      case "disputes":
        return disputes.filter(
          (d) =>
            d.match_id?.team_a_id?.name.toLowerCase().includes(query) ||
            d.match_id?.team_b_id?.name.toLowerCase().includes(query),
        );
      default:
        return [];
    }
    ...
    return (
      <div className="min-h-screen bg-light-bg flex items-center justify-center tactical-grid">
        <div className="flex flex-col items-center">
          ...
          />
          <p className="mt-6 text-[10px] font-black uppercase tracking-[0.5em] text-padel-blue animate-pulse">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-light-bg via-light-surface to-slate-50 font-sans tactical-grid pb-20">
          ...
          <div className="p-4 md:p-8">
            ...
            {filteredData().length === 0 && (
              <div className="py-20 text-center">
                <Search
                  size={48}
                  className="mx-auto text-text-tertiary mb-4 opacity-20"
                />
                <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">
                  No matching data identified in sector
                </p>
              </div>
            )}
          </div>
      ...
      <AdminModal
        isOpen={showForceMatch}
        onClose={() => setShowForceMatch(false)}
        title="Force Operational Link"
      >
        <p className="text-[10px] text-text-tertiary font-semibold mb-4">
          Copy the Team ID from the Squads tab and paste it here.
        </p>
        <form onSubmit={handleForceMatch} className="space-y-4">
          <Input
            label="Team A ID"
            value={forceMatchData.team_a_id}
            onChange={(e) =>
              setForceMatchData({
                ...forceMatchData,
          ...
          <Input
            label="Team B ID"
            value={forceMatchData.team_b_id}
            onChange={(e) =>
              setForceMatchData({
                ...forceMatchData,
          ...
          <select
            ...
            onChange={(e) =>
              setForceMatchData({ ...forceMatchData, mode: e.target.value })
            }
          >
      ...
      <AdminModal
        isOpen={showResolveDispute}
        onClose={() => setShowResolveDispute(false)}
        title="Resolve Intel Conflict"
      >
        <form onSubmit={handleResolveDispute} className="space-y-4">
          <p className="text-[10px] text-text-tertiary font-bold mb-4">
            Conflict: {selectedDispute?.match_id?.team_a_id?.name} vs{" "}
            {selectedDispute?.match_id?.team_b_id?.name}
          </p>
          <div>
            ...
            >
              ...
              <option value="VOID_MATCH">Void Match (no points)</option>
            </select>
          </div>
          <Input
            label="Final Score (e.g. 6-3, 7-5)"
            value={resolveData.final_score}
            onChange={(e) =>
              setResolveData({ ...resolveData, final_score: e.target.value })
            }
          />
          <Input
            label="Admin Notes"
            ...
            onChange={(e) =>
              setResolveData({ ...resolveData, admin_notes: e.target.value })
            }
          />
          <Button
            type="submit"
            className="w-full py-4 bg-padel-blue text-white font-black"
          >
            Finalize Resolution
          </Button>
        </form>
      ...
      <AdminModal
        isOpen={showOverrideResult}
        onClose={() => setShowOverrideResult(false)}
        title="Reality Correction"
      >
        <p className="text-[10px] text-text-tertiary font-semibold mb-4">
          Match:{" "}
          <span className="text-text-primary font-black">
            {selectedMatch?.team_a_id?.name}
          </span>{" "}
          vs{" "}
          <span className="text-text-primary font-black">
            {selectedMatch?.team_b_id?.name}
          </span>
          <br />
          Result is from <strong>Team A's perspective</strong> (WIN = Team A
          won).
        </p>
        <form onSubmit={handleOverrideResult} className="space-y-4">
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-text-tertiary mb-2">
              Result (Team A perspective)
            </label>
            <select
              className="w-full bg-light-surface border border-light-border rounded-xl px-4 py-3 text-xs font-bold focus:border-padel-blue outline-none"
              value={overrideData.result}
              onChange={(e) =>
                setOverrideData({ ...overrideData, result: e.target.value })
              }
            >
              <option value="WIN">WIN — Team A wins</option>
              <option value="LOSS">LOSS — Team B wins</option>
            </select>
          </div>
          <Input
            label="Score (e.g. 6-3, 7-5)"
            value={overrideData.score}
            onChange={(e) =>
              setOverrideData({ ...overrideData, score: e.target.value })
            }
            required
          />
          <Button
